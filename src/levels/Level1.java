...
public class Level1 {
    ...
    public static void main(String[] args) {
         ...
         */
        teachers.stream().forEach(teacher -> System.out.println(teachers));
         ...
         */
        teachers.stream().filter(teacher ->teacher.getSubject()== Subject.JAVA).distinct().sorted(Comparator.comparingInt(Teacher::getSalary)).forEach(System.out::println);
         ...
         */
        teachers.stream().filter(teacher -> teacher.getName().toLowerCase().startsWith("m")).peek(teacher -> teacher.setSalary(teacher.getSalary()+200)).max(Comparator.comparingInt(Teacher::getSalary)).ifPresent(System.out::println);
